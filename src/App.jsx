...
import { playGameOver, playVictory } from './utils/sounds';

function App() {
  const [gameState, setGameState] = useState('home'); // home, game, result, teacher, leaderboard
  ...
  return (
    <div className="arcade-container">
      {gameState === 'home' && <Home onStart={startGame} onTeacherMode={() => setGameState('teacher')} onLeaderboard={() => setGameState('leaderboard')} />}
      {gameState === 'teacher' && <Teacher customQuestions={customQuestions} setCustomQuestions={setCustomQuestions} onBack={() => setGameState('home')} />}
      {gameState === 'leaderboard' && <Leaderboard onBack={() => setGameState('home')} />}
      {gameState === 'game' && <Game questions={questions} onEnd={endGame} />}
      {gameState === 'result' && <Result score={score} total={questions.length} onHome={goHome} onTryAgain={tryAgain} userAnswers={userAnswers} submitStatus={submitStatus} />}
    </div>
  );
}
